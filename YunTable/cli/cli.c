...
private char* help(){
	char *line0 = "Welcome to YunTable\n";
	char *line1 = "Below are some sample commands:\n";
	char *line2 = "0) add master:127.0.0.1:8301\n";
	char *line3 = "1) add region:127.0.0.1:8302\n";
	char *line4 = "2) add table:people\n";
...
public void start_cli_daemon(){
	...
	if(cliCacheInst->master_conn == NULL) logg("No Master Connection has been setup\n",NULL);
	else logg("The Master connection to %s has been setup\n", cliCacheInst->master_conn);

	pthread_t fresh_thread_id;
	pthread_create(&fresh_thread_id, NULL, cli_daemon_thread, (void*)NULL);
	...
	while(1){
		...
		if(cmp(buf, QUIT_KEY, strlen(QUIT_KEY))) break;
