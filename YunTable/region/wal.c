...
public WalItem* create_wal_item(short tablet_id, long item_id, Item* item){
	WalItem *walItem = malloc(sizeof(WalItem));
...
public Wal* load_wal(char* file_path){
	...
	if(get_file_size(wal->wal_file_path) == -1) refresh_wal(wal, NULL);
	return wal;
}
...
public void refresh_wal(Wal *wal, List* walItems){
	...
	logg("creating a tmp wal file %s\n", tmp_file_path);
	...
	fclose(fp);
	logg("replace the wal file with tmp wal file\n", tmp_file_path);
